...
namespace ProjectX.UI
{
    public partial class MainForm
    {
        ...
        /// <param name="mapName"></param>
        /// <param name="utpMap"></param>
        private void OpenLayout(string mapLayoutName)
        {
            TabPage containTapPage = checkLayoutIsInUtp(mapLayoutName, utpMap);
            //若已存在，不添加,改变激活layout
            if (containTapPage != null)
            {
                activeMapLayoutControl = (MapLayoutControl)containTapPage.Controls[0];
            }
            else containTapPage= AddLayoutAndTab(mapLayoutName);
            ChangeAndOpenLayout(mapLayoutName);
            activeMapLayoutControl.MapLayout.ZoomToPaper();
            utpMap.SelectedTab = containTapPage;
        }
        /// <summary>
        /// 切换为布局界面并打开布局
        ...
        {
            ChangeToLayout();
            activeMapLayoutControl.MapLayout.Open(mapLayoutName);
        }
        /// <summary>
        /// 切换为布局界面时做的行动
        /// </summary>
        ...
        /// <param name="utpMap"></param>
        /// <returns></returns>
        private TabPage checkLayoutIsInUtp(string mapLayoutName, UserTabPage utpMap)
        {
            foreach (TabPage tabpage in utpMap.TabPages)
            {
